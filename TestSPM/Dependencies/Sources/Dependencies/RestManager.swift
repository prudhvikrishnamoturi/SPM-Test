...
//  RestManager
//
//  Created by Prudhvi on 18/05/22.
//
...
extension RestManager {
    // Manage response in general way. If required please modify according to your response model
    public struct Response {
        ...
        init(fromURLResponse response: URLResponse?) {
            ...
            httpStatusCode = (response as? HTTPURLResponse)?.statusCode ?? 0 // Return 0 means no HTTP status code could be determined
            // Fetch HTTP headers contained in the response
    ...
    // Defined custom error to provide a localized description of it
    public enum CustomError: Error {
        case failedToCreateRequest
    }
}

// Hide the underlying data storage mechanism to use as a framework
